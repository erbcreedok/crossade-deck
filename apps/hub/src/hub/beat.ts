...
/**
 * СПРАШИВАТЬ СНОВА И СНОВА, ПОКА НЕ СКАЖУТ ХВАТИТ — не анимация, а ожидание одного ответа: человек
 * ушёл в телегу привязывать аккаунт, и страница ждёт, чем это кончилось.
 *
 * Кадровым часам тут делать нечего — раз в полторы секунды, а не шестьдесят раз в секунду, — но
 * `setInterval` в хабе называется в одном файле по той же причине, по какой в одном называется
 * `requestAnimationFrame` (`hub.one-clock`): второй вызывающий «только один раз» и есть то, как
 * страница обзаводится таймерами, которых потом никто не находит.
 *
 * Возврат СНИМАЕТ ожидание, и снять его обязан тот, кто завёл: ожидание, которое не кончается, —
 * это вкладка, стучащая в сервер до конца дня.
 */
export function every(ms: number, run: () => void): () => void {
