...
| `dice.a-hard-flick-never-climbs-past-its-own-ceiling` | a die shoved off the finger at speed | `throwFromCarry`, apex tracked over the flight | its scale never climbs past `MAX_DICE_LIFT` (1.35×) of its own rest size — a shove hard enough to overshoot it is capped at the hop, not at the height |
