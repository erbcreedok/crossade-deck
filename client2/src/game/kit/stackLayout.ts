...
/** i — номер карты, n — сколько всего, cell — размер карты. Чистая функция, без Pixi. */
export type StackLayout = (i: number, n: number, cell: { w: number; h: number }) => StackOffset;
...
export function linear(opts: { angleDeg?: number; step?: number; rot?: number } = {}): StackLayout {
  ...
  return (i, _n, cell) => {
    ...
    return { dx: Math.cos(a) * step * len * i, dy: Math.sin(a) * step * len * i, rot: rot * i };
  };
}
...
export function fan(opts: FanOpts = {}): StackLayout {
  return (i, n, cell) => {
    const offs = fanAssembly(Array.from({ length: n }, (_, k) => String(k)), cell.w, opts);
    const cur = offs[i];
...
export function heap(opts: { spread?: number; tilt?: number } = {}): StackLayout {
  ...
  return (i, _n, cell) => {
    ...
    return { dx: (frac(a) - 0.5) * cell.w * spread * 2, dy: (frac(b) - 0.5) * cell.h * spread * 2, rot: (frac(a + b) - 0.5) * tilt };
  };
}
