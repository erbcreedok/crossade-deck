...
export interface DropZone {
  ...
  policy?: DropPolicy; // КАК зона ловит груз: хит-правило, вид груза, наклон, магнит (dropPolicy.ts)
}
...
// Транзиентное состояние drag-over: зарезервированный слот (дыра под падающую карту). Дети ≥ index
// раздвигаются; skip — перетаскиваемая фигура, исключается из раскладки. Не конфиг, а живое состояние.
export interface GapSpec {
  index: number;
  size: Size; // размер дыры (= ячейка/карта)
  skip?: string; // исключить эту фигуру из раскладки (её тащат)
}
...
export interface Group extends SlotBase {
  ...
  children: Slot[];
  layout: Layout;
  gap?: GapSpec; // ВРЕМЕННО во время наведения: раздвинуть детей под падающую карту
}
