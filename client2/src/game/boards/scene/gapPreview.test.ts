...
// Сторож SMART REORDER борды: гэп ставится только превьюируемым контейнерам (рука — дефолт true,
// зоны — opt-in preview:true), идемпотентно; кросс-дроп доотправляет reorder на индекс превью.
...
function harness(zones: ZoneSpec[], slots: Record<string, { members: string[] }>, handPreview = true) {
...
describe("SceneGapPreview — гэп-превью контейнеров борды", () => {
  it("рука-на-борде превьюится по дефолту: hover ставит gap и перецеливает слот", () => {
  ...
  it("кросс-дроп: в руку — reorderHand, в превью-зону — reorderSlot, оба с грузом на индексе превью", () => {
    ...
    expect(calls.cmds[0]).toEqual({ t: "reorderHand", seat: "p1", order: ["a", "x", "b"] });
