...
// Точки рождаются на переданном облаке, дрейфуют наружу и гаснут за свою жизнь (fade in/out), плюс
// мерцают по alpha. Высокая плотность + случайные фазы дают «шевелящуюся пыль».
//
// ЦВЕТ ЧАСТИЦЫ БЕРЁТСЯ У ТОЧКИ, а не задаётся полем. Раньше он был константой (амбер силуэта фака),
// и цензура выглядела одинаково поверх чего угодно — жёлтой крошкой, ни на что не похожей. Точка,
// несущая свой цвет, превращает пыль в СМАЗ того, что под ней: туз пик размазывается тузом пик.
// Точки без цвета (силуэтный путь стенда) падают на амбер, как раньше.
...
/** Точка рождения. color опционален: силуэтный путь его не задаёт и красится амбером. */
...
interface P {
  ox: number;
  oy: number; // дом (точка, где родилась)
  vx: number;
  ...
  color: number; // цвет — унаследован от точки рождения
}

export class ParticleField {
  readonly view = new Container();
  private g = new Graphics();
  private pts: DustSeed[];
  private ps: P[] = [];
  private prm: ParticleParams;

  constructor(pts: readonly DustSeed[], prm: ParticleParams) {
  ...
  private seedAll(t: number): void {
    this.ps = this.pts.map(() => {
      const p: P = { ox: 0, oy: 0, vx: 0, vy: 0, born: 0, life: 0, ph: 0, color: AMBER };
      this.spawn(p, t);
      return p;
    });
  }
  ...
  /** Пересадить облако на другое лицо: пыль обязана следовать за тем, что под ней. */
  setPoints(pts: readonly DustSeed[]): void {
    this.pts = pts.length ? [...pts] : [{ x: 0, y: 0 }];
    this.seedAll(0);
  }
  ...
  update(t: number): void {
    ...
    for (const p of this.ps) {
      ...
      g.rect(x - s / 2, y - s / 2, s, s).fill({ color: p.color, alpha: a });
    }
  }
