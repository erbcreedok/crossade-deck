...
## Последнее — сессия 2026-07-31 (общий слой сцены + канвасная обвязка)
...
**Что дальше:** Косынка садится на `SceneEngine` — ввод → камера → драг → слоты → раскладка
(§3 REBUILD-хендоффа), и её экраны переезжают на `ui/TopBar`. Начинать с задачи **#96**, там
в комментарии расписано откуда.
