...
## 2. Design-доки — УДАЛЕНЫ (и не возвращать)

`SOLITAIRE-MVP-PLAN.md`, `-ARCHITECTURE.md`, `-TECHNICAL-SPEC.md`, `-OVERVIEW.md`, `-INDEX.md`
(3163 строки) лежали в `client2/` и снесены вместе с переписью. Причина не в объёме: они
описывали проект, которого нет и не будет — `mountSolitaireBoard`/`updateBoardVisuals`,
`slotGeometries`, три viewport-профиля, веерную раскладку колонок, 14 слотов вместо 13, ссылки на
`client2/src/FreeDesk.tsx` (файл остался в старом `client/`). **Именно буквальное следование этим
спекам и породило заход, который владелец не принял.** Держать рядом с рабочим кодом 3000 строк
неверного ТЗ — это ловушка для следующего агента, а не «более широкий контекст».

Что от них осталось живого и где теперь лежит:

| Было в доках | Где сейчас |
|---|---|
| правила Клондайка | `board/solitaireRules.ts` + тесты — исполняемая правда |
| модель состояния и раздача | `board/solitaireState.ts` + тесты |
| геометрия доски | `solitaire/tree.ts` + тесты |
| как устроен рендер и ввод | `solitaire/scene.ts` и общий `engine/sceneEngine.ts` |
| что и как проверять | [`SOLITAIRE-REBUILD-HANDOFF.md`](./SOLITAIRE-REBUILD-HANDOFF.md) §5–§6 + `e2e/solitaire.spec.ts` |

Историю никто не отменял: доки в git (`git log --diff-filter=D -- client2/SOLITAIRE-*.md`).
`MINI-GAMES-SUITE.md` (5 доп. пасьянсов после MVP) в репо никогда не завозился — он остался на
ветке `claude/solitaire-mvp-design-jo2akg`, коммит `7b99174`.
...
`solitaire/app.ts`, `solitaire/preset.ts` (дублировал `createInitialState()` и никем не
импортировался).
...
**Что осталось за владельцем:** приёмка ветки и пустая треть под доской на телефоне (вписывание
упирается в ширину).
...
Исходная цепочка зависимостей и оценок жила в `SOLITAIRE-MVP-PLAN.md` §XI — док удалён (§2), но
доступен в истории git. Полагаться на него не стоит: сроки и состав эпиков он описывал для
проекта, которого нет.
