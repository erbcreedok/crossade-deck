...
## 3.3. ГДЕ МЫ СЕЙЧАС (2026-07-31): общий слой готов, пасьянс садится на него

**Читать перед любой правкой по пасьянсу:** [`SOLITAIRE-REBUILD-HANDOFF.md`](./SOLITAIRE-REBUILD-HANDOFF.md) —
чекпоинт, разбор брака, целевая архитектура, ловушки (§5) и обязательный порядок проверки (§6).

Состояние: **§3 (разделение shared/личного) и §4 (обвязка экрана) ЗАКРЫТЫ и запушены.**

- **`engine/sceneEngine.ts`** — общая обвязка сцены: камера, ввод, драг, дроп-зоны, «подглядеть»,
  цикл и тени, экранный HUD. Вынесена рефакторингом из `playgroundEngine.ts`; песочница на ней
  зелёная (e2e 161/161). Пасьянс встаёт на неё же — **обрасти своим вводом и камерой ему больше
  неоткуда.**
- **`ui/TopBar.ts`** — канвасная верхняя панель. Решение владельца: HTML в приложении быть не
  должно, всё рисует движок (переносимость на платформы без DOM). Экраны Косынки переезжают на
  неё вместе с переписью визуального слоя.
- `peek.spec.ts:73` решён (прав код), `test-results/` в `.gitignore`.

**Следующий шаг — задача #96**, в ней комментарий с разбором: что брать из общего слоя, какая
часть её собственной спеки недействительна и почему. Порядок: ввод → камера → драг → слоты →
раскладка. Логику (E1/E2, `/no-ui`) не трогать.
...
(запушен 2026-07-31 вместе с началом переделки — см. §3.3). Игра открывается на `/solitaire` и на кнопку «косынка» в меню:
...
Оба сейчас в статусе **In Progress** на борде: они переделываются вместе с раскладкой. Сам
эпик #77 возвращён из «Ready to merge» в «In Progress» — файл, которым он владеет
(`board/solitaireLayout.ts`), по §3 REBUILD-хендоффа подлежит УДАЛЕНИЮ в пользу `slot/layouts`,
а «14 слотов» в его Goal — ошибка спеки, слотов 13. Подробности — в комментарии на #77.
