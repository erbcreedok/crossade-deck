...
## POC (сделано): доказательство ядра вживую

`src/game/path2Scene.ts` + `Path2Demo.tsx`, открывается по `#path2`. Отдельная Pixi-сцена на
`TablePool`: тап по карте перегоняет её в следующий бокс, и это ТОТ ЖЕ спрайт, летящий
пружиной, а не пересоздание. Де-риск: подтверждает, что единый пул + `setTarget` дают
бесшовный перелёт бокс→бокс. (Запуск: dev-сервер client2 → `/#path2`.) Боевой движок не
трогает. Осталось: перенести этот же принцип в RoomEngine (рецепт ниже) — с проверкой на
устройстве.
