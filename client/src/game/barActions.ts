...
export function barActionsFor(_sel: Selection, ctx: BarContext): BarActions {
  ...
  if (ctx.amIDealer) {
    // Пустую колоду мешать не в чем: кнопка осталась бы яркой и молчала бы в ответ на
    // нажатие. Пустой слот панель показывает как неактивную кнопку и каркас не двигает
    // (см. ActionBar), поэтому «ГОУ!» никуда не переезжает. Счётчик, которого нет,
    // пустотой НЕ считается — вызывающий мог просто не передать его.
    const emptyDeck = ctx.deckCount === 0;
    return {
      main: emptyDeck ? null : { id: "shuffle", label: "Перемешать" },
