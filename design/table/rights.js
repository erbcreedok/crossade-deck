...
//
//   "table"   — управление ЧЕЛОВЕКОМ ЗА ЭТИМ СТОЛОМ: стул, права, лок/пин/скрыть, его карты, кик.
...
(function () {
  ...
  const RULES = [
    ...
    {
      id: "cards:peek", label: "Смотреть карты", where: ["table"], kind: "",
      // ЧУЖАЯ РУКА — САМОЕ ГРОМКОЕ ДЕЙСТВИЕ ЗА СТОЛОМ. Оно есть, потому что стол домашний и
      // ведущий иногда обязан подсмотреть; но оно не бывает тихим — экран того, чью руку открыли,
      // говорит об этом вслух, иначе это не право, а жульничество с кнопкой.
      may: (me, them, room, p) => them.mine ? "свои и так видно" : !them.seat ? "он не за столом" : manages(p) || "чужую руку открывает тот, кто распоряжается",
    },
    {
      id: "cards:take", label: "Стянуть карту", where: ["table"], kind: "",
      may: (me, them, room, p) => them.mine ? "это твои карты" : !them.seat ? "он не за столом" : manages(p) || "тянуть из чужой руки может тот, кто распоряжается",
    },
    {
