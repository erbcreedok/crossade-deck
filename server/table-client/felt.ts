...
/**
 * СТРЕЛКА ОЖИДАНИЯ: её размеры и то, где она сидит.
 *
 * `out` — насколько её ОСНОВАНИЕ вынесено за кромку круга. Почти ноль: стрелка стоит НА ободе, а
 ...
 */
const TURN_MARK = { out: 0.1, w: 1.15, h: 1.25, line: 0.22 };
...
function turnMark(g: CanvasRenderingContext2D, ink: string): void {
  ...
  g.save();
  // Рисуется в осях круга: -y — наружу, к тому, кого ждут; +y — в середину круга. Основание сидит
  // на ободе, остриё смотрит на игрока.
  g.beginPath();
  // Древко — короткое и толстое: стрелка должна читаться через весь стол, а не только вблизи.
  g.moveTo(0, h * 0.5);
  g.lineTo(0, -h * 0.05);
  g.lineWidth = line;
  g.strokeStyle = ink;
