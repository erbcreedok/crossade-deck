...
describe("круг хода стоит на месте", () => {
  ...
  it("счёт снятых снизу обнуляется вместе с опустевшим кругом", () => {
    const t = ringTable();
    intoRing(t);
    intoRing(t);
    ...
    expect(ring(t)!.taken, "нижнюю сняли — дыра").toBe(1);
    t.act("Аня", { t: "deckMove", pile: RING, x: 3, y: 3 }, 0);
    expect(ring(t)!.taken, "круг разобрали целиком — счёт с нуля").toBe(0);
  });
