...
export function openEntry(room: string, home: Home, by: string, title?: string, now = Date.now(), kind: string = DEFAULT_DESK, crew?: string): RoomCard {
  ...
  if (had) {
    ...
    // безымянная и ничья; бот приходит следом и забирает своё: имя, дом, права админа И РОД СТОЛА.
    //
    // РОД ВОЗВРАЩАЕТСЯ ОБЯЗАТЕЛЬНО. Вошедший рода не знает — он открывает дверь, а не стол, — и
    // комната заводится песочницей. Пока род не возвращали, крестовая комната, поднятая чужим
    // входом, оставалась песочницей НАВСЕГДА: та же ссылка, то же место в чате, а за столом нет ни
    // круга хода, ни правил, по которым в ней играли. Владелец так и написал: захожу по ссылке на
    // крестовый — попадаю в песочницу.
    if (had.by === "" && by) {
      had.by = by;
      had.home = home;
      if (title?.trim()) had.title = uniqueTitle(title.trim(), takenTitles(room));
      if (isDesk(kind) && kind !== had.kind) {
        had.kind = kind;
        had.crew = isCrew(crew) ? crew : (deskCrew(kind) ?? had.crew);
        had.live?.recast?.(kind);
      }
      had.live?.claim?.(by);
      save(had);
    }
