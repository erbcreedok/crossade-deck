...
export class Table {
  ...
  /**
   * СКАЗАТЬ ЗА СТОЛОМ — событие, которое состояния не меняет, но должно дойти до всех.
   *
   * Такому событию нужна СВОЯ ВЕРСИЯ: клиент принимает патч, только если он новее того, что у него
   * есть, — и объявление, посланное со старой версией, он справедливо выбрасывал. Так пропало
   * «раздал»: в хвосте для вошедшего оно было, а на живом экране его не видел никто.
   */
  say(op: Op): Op[] {
