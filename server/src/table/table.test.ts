...
describe("Table: флаги и права", () => {
  it("свои флаги — хозяин; ЧУЖИЕ — РАСПОРЯДИТЕЛЬ; покинутого — любой", () => {
    ...
    expect("ops" in t.act("admin", { t: "flag", chair: b, flag: "lock", on: false }, 0), "распорядитель снимает чужой замок: тот переживает хозяина").toBe(true);
  ...
  it("РОЛЬ ДЕРЖИТСЯ ЗА ЧЕЛОВЕКОМ: вышел — осталась, вернулся — снова с нею", () => {
    ...
    t.join(person("admin"));
    expect(t.seenBy("b").admin).toBe("admin");
    // Права вернулись вместе с человеком: чужие флаги ему снова подвластны, соседу — нет.
    expect("ops" in t.act("admin", { t: "flag", chair: b, flag: "lock", on: true }, 0)).toBe(true);
  });

  it("лок: чужие не берут и не кладут, хозяин — свободно, а СНЯТЬ может хозяин и распорядитель", () => {
...
describe("патч клиента совпадает с сервером", () => {
  it("после каждого шага снимок + дифы == снимок, взятый целиком, для каждого зрителя", () => {
    ...
    step("a", { t: "gather", ids: t.seenBy("a").piles.find((p) => p.id === "deck")!.cards.slice(0, 2).map((c) => c.id), side: "down", to: { x: 1, y: -3, angle: 0 } });
    step("b", { t: "pileDrop", pile: "p2", to: { in: "deck", pile: "p1", i: 0 } });
    step("a", { t: "deckGuard", pile: "p1", guard: "seal", on: true });
    step("a", { t: "deckGuard", pile: "p1", guard: "seal", on: false });
