...
export interface DeckSpot {
  ...
  name?: string;
  /**
   * ЯКОРЬ КОЛЬЦА — угол, на котором стоит стрелка, и от которого зона раздаёт места.
   *
   * ЕДИНСТВЕННОЕ, ЧТО ЗОНА ПОМНИТ. Не список мест — места лежат у самих карт, — а одно число, и
   * вывести его из карт нельзя: он обязан пережить изъятие головы, иначе круг провернётся целиком
   * оттого, что кто-то взял одну карту.
   */
  turn?: number;
  /**
   * НА СКОЛЬКО МЕСТ КРУГ РАЗЛОЖЕН СЕЙЧАС — второе и последнее, что помнит зона.
   *
   * Взяли карту — число не меняется, и на её месте остаётся честная ДЫРА: место без карты. Выводить
   * дыры из промежутков между картами нельзя — на тесном круге промежуток почти не виден, а если
   * забрать каждую вторую, круг сочтёт себя просто разрежённым. Число обновляется только раскладкой.
   */
  slots?: number;
  /**
