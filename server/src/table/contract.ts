...
export type Where =
   ...
   *
   * `at` — ТОЧНОЕ МЕСТО ВНУТРИ ЗОНЫ: дыра в круге или то место, откуда карту и взяли. Зона его
   * записывает как есть и НИЧЕГО не перекладывает: перекладывание — ответ только на смену порядка.
   */
  | { in: "deck"; pile: string; i?: number; at?: Laid }
...
export interface SeenCard {
  ...
  up?: boolean;
  /**
   * СВОЁ МЕСТО ВНУТРИ ЗОНЫ — записанное, а не вычисленное. Есть у карт зон с раскладкой (круг хода):
   * положили — метод зоны записал, и дальше оно просто лежит. Никто его не пересчитывает, пока в зону
   * снова не положат карту, поэтому взятая карта не двигает соседей, а на её месте остаётся дыра.
   */
  at?: Laid;
}
