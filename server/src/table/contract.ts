...
export interface DeckSpot {
  ...
  name?: string;
  /**
   * МЕСТ В КОЛЬЦЕ — по скольким точкам расставлены карты. Только для позы `ring`.
   *
   * Отдельное число, а не «сколько карт лежит», именно затем, чтобы карты НЕ ЕЗДИЛИ: положили ещё
   * одну — остальные остались на своих местах.
   */
  seats?: number;
  /**
   * СКОЛЬКО КАРТ СНЯЛИ СНИЗУ с тех пор, как зона в последний раз опустела.
   *
   * По нему считается АБСОЛЮТНОЕ место карты в кольце: `снятые + её номер в стопке`. Без него
   * взятая нижняя сдвинула бы всех остальных на одно место — а по правилу мастодонта кольцо должно
   * РВАТЬСЯ: дыра и есть след того, что кто-то взял.
   */
  taken?: number;
  forever: boolean;
