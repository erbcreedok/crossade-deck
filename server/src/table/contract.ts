...
export type Where =
   ...
   *
   * `slot` — НОМЕР СВОБОДНОГО МЕСТА ЗОНЫ: дыра в круге или то место, откуда карту и взяли. Зона сажает
   * карту на этот номер и НИЧЕГО не перекладывает: перекладывание — ответ только на смену порядка.
   */
  | { in: "deck"; pile: string; i?: number; slot?: number }
...
export interface SeenCard {
  ...
  up?: boolean;
  /**
   * НОМЕР СВОЕГО МЕСТА В ЗОНЕ — и это ВСЁ, что карта о своём положении знает.
   *
   * Координат в состоянии нет ни у кого: где место с таким номером лежит на столе, считается из угла
   * зоны и числа её мест — одной функцией, одинаково у стола, у превью и у отрисовки. Записанная
   * координата была бы вторым источником правды, и он однажды разойдётся с первым.
   *
   * Взяли карту — её номер освободился, у соседей номера прежние: вот и дыра.
   */
  slot?: number;
}
