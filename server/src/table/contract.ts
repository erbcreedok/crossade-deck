...
export interface DeckSpot {
  ...
  name?: string;
  /**
   * МЕСТО КАЖДОЙ КАРТЫ В КОЛЬЦЕ — по одному числу на карту, в порядке `cards`.
   *
   * Место ЗА КАРТОЙ, а не за её номером в стопке: взяли любую — остальные не шелохнулись, а на её
   * месте осталась дыра. Она и есть след того, что кто-то взял. Пересчитываются места только при
   * ДОБАВЛЕНИИ карты: круг сомкнулся и разложился заново.
   */
  slots?: number[];
  forever: boolean;
