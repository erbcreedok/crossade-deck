...
/**
 * ГДЕ ЛЕЖИТ КРУГ КАРТ — на полпути от середины до контура, пока карты не начали тесниться.
 *
 * Кольцо у самого контура читается как ободок, а не как ход: карты далеко друг от друга и далеко от
 * глаза. Поэтому по умолчанию они собраны вдвое ближе к середине, а РАЗЪЕЗЖАЮТСЯ только когда мест
 * стало столько, что соседи налезают друг на друга. Дальше контура не уходят: там уже не круг хода.
 */
export const RING_HOME = RING_CARDS / 2;
