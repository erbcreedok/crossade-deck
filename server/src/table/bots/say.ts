...
export function ask(legal: readonly Move[], view: BotView, profile: Profile): string {
  ...
  return [
    // ЦЕЛЬ — ПЕРВОЙ СТРОКОЙ. Без неё модель знает, ЧЕМ бить, но не знает, ЗАЧЕМ, и выбирает
    // по-разному на одном и том же столе. С названной целью ответы стали одинаковыми, а время не
    // изменилось: его съедает запуск программы, а не длина вопроса.
    "Ты играешь в крестового дурака (мастодонт). ЦЕЛЬ: первым избавиться от всех своих карт.",
    "Кто остался с картами последним — проиграл. Кладёшь карту в круг — избавляешься от неё;",
    "берёшь из круга — карт становится больше, и до победы дальше.",
    "Козырь — буби. Крести бьются только крестями. Джокер бьёт всё, но любая шестёрка бьёт джокера.",
