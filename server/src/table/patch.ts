...
function applyOp(s: Snapshot, op: Op): void {
  switch (op.t) {
    ...
    case "play":
      s.play = op.play;
      return;
    // ОБЪЯВЛЕНИЕ РАЗДАЧИ СОСТОЯНИЯ НЕ МЕНЯЕТ: карты уже разъехались своими движениями. Это слово для
    // журнала — «раздали, и вот кому сколько», сказанное тем, кто раздавал.
    case "dealt":
      return;
  }
}
