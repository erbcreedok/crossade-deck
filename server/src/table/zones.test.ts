...
import { RING_CARDS, RING_HOME, RING_SPREAD, ringHoles, ringLay } from "./ring.js";
...
describe("раскладка круга — метод, а не формула", () => {
  ...
  it("МЕСТ НЕ МЕНЬШЕ ТРЁХ: одна карта стоит в круге на три, а не сама по себе", () => {
    ...
    // Вторая встанет через 120°, а не вплотную: круг читается как круг с первой же карты.
    const two = ringLay(mid, 2);
    expect(Math.atan2(two[1]!.x, -two[1]!.y) * 180 / Math.PI).toBeCloseTo(120, 4);
  });

  it("МЕСТА ДЕЛЯТ КРУГ ПОРОВНУ, а карта смотрит верхом в середину", () => {
    const four = ringLay(mid, 4);
    const turn = (p: { x: number; y: number }) => ((Math.atan2(p.x, -p.y) * 180 / Math.PI) + 360) % 360;
    expect(four.map(turn).map(Math.round)).toEqual([0, 90, 180, 270]);
  ...
  it("ЯКОРЬ — УГОЛ ПЕРВОЙ КАРТЫ: круг не проворачивается целиком от каждого перекладывания", () => {
