...
export interface PersonAtTable {
  ...
  /**
   * МЕСТО В ИДУЩЕЙ СЕССИИ — оно и значит «за столом». Не роль: роль живёт в базе и переживает
   * сессию, а стул существует ровно столько, сколько идёт партия, и увидеть его можно только здесь.
   */
  readonly seat: string | null;
  /** Чей это аккаунт. Пусто — сел гость, и связать его с записью в базе не с чем. */
