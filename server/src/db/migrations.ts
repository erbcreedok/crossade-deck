...
export const MIGRATIONS: readonly Migration[] = [
  ...
  },
  {
    version: 18,
    up(db) {
      // КОМНАТЫ СТОЛА ПЕРЕЖИВАЮТ ПРОЦЕСС. Раньше они жили только в памяти: перезапуск или выкатка — и
      // столов нет, ни записи (чей, как зовётся, какой игры), ни самих карт.
      //
      // Две вещи в одной строке, и меняются они порознь: `card` — запись комнаты (JSON, правится редко),
      // `state` — слепок стола и партии (JSON, пишется, когда стол затих). Состояние без записи не
      // бывает; запись без состояния — комната, в которую ещё никто не входил.
      db.exec(`
