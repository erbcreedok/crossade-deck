...
/**
 * КОД — СЛУЧАЙНЫЙ И НЕ ДЛЯ ЧТЕНИЯ ВСЛУХ: его подставляет ссылка, а не человек.
 *
 * HEX, А НЕ base64url, И ЭТО НЕ ВКУСОВЩИНА: в ссылке перед кодом едет имя приложения, отделённое
 * `_` (`<source>_<код>`), а base64url сам содержит и `_`, и `-` — разделитель перестал бы быть
 * разделителем ровно в тот день, когда очередной код выпадет с подчёркиванием.
 */
