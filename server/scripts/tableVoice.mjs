...
{
  // У сервера пути для речи нет вовсе: он сводит двоих запиской (`MSG.rtc`), и больше о голосе не знает.
  const health = await (await fetch(`${base}/health`)).json();
  check("через сервер речь не идёт вовсе — у него и пути такого нет", health.status === "ok" && health.live === undefined, health);
}
