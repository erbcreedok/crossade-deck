...
// Карту обратно в руку — круг снова пуст. Она лежит НА КРУГЕ, а не в его середине: в середине
// теперь ручка, и тянуть оттуда значит тянуть стопку.
const RING_CARDS = 3 - 1.4 / 2;
const onRing = { x: mid.x, y: mid.y - RING_CARDS * (await spots()).k };
await p.mouse.move(onRing.x, onRing.y);
await p.mouse.down();
await p.mouse.move(195, 800, { steps: 8 });
await p.mouse.up();
await p.waitForTimeout(600);
check("круг опустел — ручка снова спряталась", (await ringGrip()) === null, await grips());
