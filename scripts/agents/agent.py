...
def watch(name, model, wt: Path, d: Path, prompt: str, max_steps: int, max_tokens: int):
    ...
    proc = subprocess.Popen(
        ...
        # ФОНОВЫЙ СЕРВЕР АГЕНТА НЕ ПОВОД ЕГО УБИВАТЬ. Claude Code в режиме `-p` через 600 с после
        # ответа режет всё, у чего ещё жив фоновый процесс, — и агент, поднявший storybook ради
        # скриншотов, умирал с незакоммиченной работой. Потолок ожидания снят; кто не убрал за
        # собой сервер — того убьёт `kill` по бюджету, а не оболочка на полпути.
