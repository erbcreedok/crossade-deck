...
`Transformable.angle`. Атом хранит и упоры, и то, на каком узел СТОИТ (`stop`) — состояние на том же
атоме, что и спека, ровно как `Flippable.turns`. До этого упор жил в рантайме и нигде больше: наклон
не переживал перезагрузку и не был виден второму клиенту. Угловой близнец `Flippable`. Дизайн —
`CANONS.md` §3.
