...
`vitest + jsdom` · 69 кейсов, расписано 69
...
| id | Дано | Когда | Тогда |
|---|---|---|---|
| `toolbar.answers-the-SECOND-finger` | тулбар сцены, палец с чужим id | down/up по кнопке | кнопка отвечает ЛЮБОМУ пальцу: совместимостный `click` спецификация даёт только ПЕРВИЧНОМУ указателю, поэтому при руке на колоде вся консоль молчала — и метку поставить было нечем |
| `toolbar.a-press-fires-ONCE` | первичное касание, за которым идёт синтезированный клик; затем клик с клавиатуры | нажатие | нажатие срабатывает один раз: синтезированный клик глушится на `pointerdown`, а путь `click` оставлен клавиатуре (`detail === 0`) |
| `toolbar.a-finger-that-leaves-does-not-press` | down по кнопке, затем cancel; и up ЧУЖИМ пальцем | нажатие | ни отменённый жест, ни чужой палец не считаются нажатием |
| `toolbar.every-button-is-big-enough-for-a-thumb` | кнопки ряда | измерение | цель не ниже 34px: этот ряд жмут на том же стекле, где стол, одной рукой |
...
| `controls.the-glass-answers-where-a-held-thing-is` | исходник страницы жестов | скан вызовов `worldAt` | дерево спрашивают ТОЛЬКО про место за столом: место прибито к сукну, а всё, что рука может поднять, спрашивают у стекла (`drawnAt`/`packAt`) — перенос дерево не пишет и называет точку, ОТКУДА подняли |
