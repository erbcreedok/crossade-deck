...
`vitest (headless, no WebGL)` · 24 кейсов, расписано 24

| id | дано | когда | тогда |
|---|---|---|---|
| `overlap.two-shapes-touch-when-no-line-fits-between-them` | две карты: врозь, внахлёст, встык, на волосок врозь | вопрос о касании | разделяющая ось по контурам: встык — это касание, волосок — нет, пока вызывающий не назовёт допуск; отрицательный допуск требует настоящего нахлёста |
| `overlap.a-BOX-would-lie-about-a-turned-card` | две карты под 45°, угол к углу | вопрос о касании | спрашивают КОНТУРЫ, а не габариты: коробки пересекаются, карты — нет, и стопка, собранная по коробкам, сгребла бы то, что игрок видит раздельно |
| `overlap.nothing-is-inside-a-line` | точка, отрезок, пустой контур | вопрос о касании | у фигуры без площади нет внутренности: ничего не касается |
| `islands.a-chain-is-ONE-pile` | цепочка из шести, тройка, пара и одиночка | обход | касание транзитивно: четвёртая карта, положенная на третью, вошла в стопку, начатую первыми двумя. Порядок групп — порядок появления, так что неизменный стол отвечает одинаково |
