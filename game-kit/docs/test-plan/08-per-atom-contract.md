...
`vitest` · 109 кейсов, расписано 109
...
| `stage.a-frame-walks-the-tree-once` | стол на 100 и на 1600 узлов, каждый Bakeable; кадр через `attachPainter` + `setRoot`, лучший из пяти | отношение времени кадра | меньше 40 при шестнадцатикратном дереве: линейный закон даёт около 16, квадратный — за сотню, и окно между ними шире любой заминки машины. Предикат запекания спрашивает ВЛАДЕЛЬЦА квада; поиск по id на каждый квад был обходом всего дерева — 200 обходов по 200 узлов на кадр, крупнейшая статья переноса на телефоне. Одна карта владельцев на кадр |
...
| `stage.what-the-finger-holds-paints-over-the-screen` | стол с картой (тень) и второй фигурой, на hud-корне картинка | кадр в покое; с `raised` карты; с `raised`+`carried` карты | в покое экран поверх стола; в полёте, но не в руке — по-прежнему под экраном; в руке — карта ПОВЕРХ экрана и её тень с ней, остальной стол на месте: что в руке, ближе всего к глазу, а картинка над картами руки читалась бы как «подсунули сзади» |
...
| `plan.a-screened-control-casts-no-shadow` | диск аватара / кольцо места (`Screened`) и рядом карта | план | у контрола нет shadow-квада, у карты — есть |
...
| `pose.a-stack-has-a-thickness` | `stack(52)`, `stack(500, { thickness })` | прочитаны `at` | подъём всей стопки не выше `thickness` (0.18 по умолчанию): сдвиг ужимается одним вектором, направление и ровный шаг сохраняются — 500 карт это колода, а не лестница |
