...
| `accept.a-rule-can-read-the-zones-own-state` | правило замка: `{or:[{not:{eq:["target.values.lock",1]}},{eq:["actor.seat","south"]}]}` | зона с `lock:1`, с `lock:0` и вовсе без значений — от чужого и от владельца | закрытая: чужому `deny`, владельцу `allow`; открытая — `allow`; без `Valued` вовсе — `allow`, путь отсутствует, сравнение с отсутствующим — тихое нет, отрицание над ним читается как открытая дверь. Значения ЭЛЕМЕНТА читались с самого начала, значения контейнера — нет: пока у зоны не было своего состояния, всякое правило было про предлагаемую вещь. Рука с замком — второй спрашивающий, и без пути замок пришлось бы пересобирать кодом при каждом повороте |
