...
`vitest + fs scan (like argNames.test.ts)` · 36 кейсов, расписано 36
...
| `guard.one-run-out-law` | всё дерево `src`, кроме `core/glide.ts`, самих тестов и каталога | скан КОДА (не прозы) на числа `0.99` / `0.998` / `0.999`; плюс вопрос к `DEFAULT_TUNING` | платформенная ставка торможения записана ровно в одном файле: Swift-сторона ставит `decelerationRate` тем же числом, а второй вывод где-то ещё разойдётся с первым. Тюнинг называет ЗАКОН именем, а не носит число: своё сукно игра регистрирует (`registerGlide`) — та же дверь, что у сглаживаний и тасовок |
...
| `guard.one-door-per-folder` | всё дерево `src/` вне папок-с-дверью (`render/pixi/`, `render/animator/`, `render/scenePlan/`, `render/camera/`) | скан импортов, целящих внутрь этих папок | ни один не проходит мимо их `index.js`. Это и делает папку файлом: как только снаружи потянули `pixi/same.js`, дверь перестала быть одна, и «поменять рендерер» перестало быть одной правкой на границе. Обе папки появились потому, что их единственный файл перевалил за тысячу строк, — и раскол остаётся улучшением ровно пока папка отвечает как ОДНА вещь |
