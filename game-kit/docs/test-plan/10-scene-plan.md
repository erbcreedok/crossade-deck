...
| `plan.a-shadow-follows-its-caster-in-flight` | кастер с override-позой (полёт) | план с overrides | тень сдвинута ровно вместе с кастером: полёт несёт и предмет, и его тень |
...
| `plan.flight-deepens-the-shadow` | кастер в `raised` и без | длина падения сверена | полёт ЕСТЬ высота: поднятому падение удлиняется тем же raised-хинтом, что поднимает порядок покраски, и кончается вместе с полётом — арифметика покоя не тронута |
