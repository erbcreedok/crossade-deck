...
export function scenePlan({ root, unit, width, height, viewer, view, pitch, overrides, raised, carried, grounded, measure, now }: PlanInput): Quad[] {
  ...
  const visit = (n: Node): void => {
    ...
    // A mark rides its piece the way a shadow does: raised with it, and never left on the desk.
    for (const q of markQuads(n, ctx, markCtx)) {
      out.push(q);
  ...
  //
  // MARKS make it seven: a badge is painted over its own piece and under nothing at that height, so
  // each height reads shadows, then pieces, then marks — and a raised piece's mark rises with it.
