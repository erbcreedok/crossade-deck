...
import { add, byId, caps, compose, fieldsOf, remove, type Node, type NodeId } from "../core/node.js";
...
export function wireDrag<S extends DragScene = DragScene>(s: S, opts: DragOptions = {}): S {
  ...
  const onDown = (e: PointerEvent): void => {
    ...
    motions.grab(items, {
      ...
      onSnap: (_ids: readonly NodeId[], at: Vec) => {
        const taken = w.drag;
        w.drag = undefined;
        w.undoInvites?.();
        w.undoInvites = undefined;
