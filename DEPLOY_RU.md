...
Требования: **Node 22+**, git. Больше ничего — ни сервера базы, ни Redis. Аккаунты, комнаты столов и
журнал лежат в одном файле SQLite, `server/data/crossade.db` (`node:sqlite`, без нативной сборки).
...
> Комнаты столов (`/table/`) рестарт переживают: запись комнаты и слепок стола лежат в
> `server/data/crossade.db`, люди возвращаются сами и садятся на свои стулья. Сессии `card_room` /
> `kit_room` и инвайт-коды по-прежнему живут **только в памяти** — их посреди игры не обновлять.

## 7. Бэкап базы

`server/data/` в `.gitignore`, `git pull` его не трогает. Раз в сутки в `crontab -e` — через
`.backup`, а не `cp`: сервер пишет в режиме WAL, и простая копия живого файла может выйти рваной:
