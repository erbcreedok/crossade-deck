...
Перед выкаткой скрипт проверяет, что образ читается АНОНИМНО: Fly тянет его именно так, и
приватный пакет иначе даёт невнятную ошибку доступа уже в процессе.
...
Выкатки на `main` НЕТ намеренно. `main` производит артефакт, прод меняется отдельным
действием. Цена — лишний клик; взамен прод не едет сам по себе, а откат становится тегом,
а не revert'ом.
