...
const card = (room: string, title: string, by = "tg:1"): RoomCard => ({ room, title, by, home: { kind: "chat", chat: "-1" }, people: [], seats: [], createdAt: 0, kind: "sandbox", crew: "sandbox", admins: [] });
...
describe("слова бота про комнаты", () => {
  ...
  it("в личке видно, где стол живёт: чат по имени, переписка — «в переписке»", () => {
    const inChat: RoomCard = { room: "r1", title: "«Пицца»", by: "tg:1", home: { kind: "chat", chat: "-1", chatTitle: "Пицца" }, people: [], seats: [], createdAt: 0, kind: "sandbox", crew: "sandbox", admins: [] };
    const inline: RoomCard = { room: "r2", title: "«Ржавый обоз»", by: "tg:1", home: { kind: "inline", message: "m" }, people: [], seats: [], createdAt: 0, kind: "sandbox", crew: "sandbox", admins: [] };
  ...
  it("готовый стол карточкой в чужую переписку: админу — «Управлять», остальным только вход", () => {
    const one: RoomCard = { room: "r1", title: "«Пицца»", by: "tg:1", home: { kind: "chat", chat: "-1", chatTitle: "Пицца" }, people: [], seats: [], createdAt: 0, kind: "sandbox", crew: "sandbox", admins: [] };
