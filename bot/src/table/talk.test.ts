...
describe("слова бота про комнаты", () => {
  ...
  it("в личке видно, где стол живёт: чат по имени, переписка — «в переписке»", () => {
    const inChat: RoomCard = { room: "r1", title: "«Пицца»", by: "tg:1", home: { kind: "chat", chat: "-1", chatTitle: "Пицца" }, people: [], createdAt: 0, kind: "sandbox" };
    const inline: RoomCard = { room: "r2", title: "«Ржавый обоз»", by: "tg:1", home: { kind: "inline", message: "m" }, people: [], createdAt: 0, kind: "sandbox" };
  ...
  it("готовый стол карточкой в чужую переписку: админу — «Управлять», остальным только вход", () => {
    const one: RoomCard = { room: "r1", title: "«Пицца»", by: "tg:1", home: { kind: "chat", chat: "-1", chatTitle: "Пицца" }, people: [], createdAt: 0, kind: "sandbox" };
