...
| `design/` | local design stands — plain HTML pages with knobs the owner tunes a screen on before it is built (`design/hud`: the hand and the rights at the bottom; `design/tophud`: the strip at the top; `design/hubhome`: the hub's first page, profile and the way in); served by `*-stand` entries in `.claude/launch.json` | the page itself |
